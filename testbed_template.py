...
#    controllers = os.environ.get('CONTROLLERS', '127.0.0.1').replace(' ', '').split(',')
#    all_controllers = []
#    for i,val in enumerate(controllers):
#        cog.outl("host%s = 'root@%s'" %(i+1, val))
#        all_controllers.append('host%s' %(i+1))
#    cog.outl("all_controllers = %s" %(all_controllers))
#]]]
#[[[end]]]
...
env.roledefs = {
    'all': all_controllers,
    'cfgm': all_controllers,
    'openstack': [host1],
    'control': all_controllers,
    'compute': [],
    'collector': all_controllers,
    'webui': all_controllers,
    'database': all_controllers,
    'build': [host_build],
